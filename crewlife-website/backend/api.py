#!/usr/bin/env python3
"""
Flask API для CrewLife Admin Panel
...
def login():
    ...
    try:
        ...
        with sqlite3.connect(db.db_path) as conn:
            ...
            ''', (user['id'], 'login', f"Вход в систему: {user['first_name']} {user['last_name']}"))
...
def create_user():
    ...
    try:
        ...
        # Валидация данных
        required_fields = ['employee_id', 'first_name', 'last_name', 'position', 'location', 'email', 'password']
        for field in required_fields:
            if not data.get(field):
                return jsonify({'error': f'Поле {field} обязательно'}), 400
        
        # Проверка уникальности email и employee_id
        with sqlite3.connect(db.db_path) as conn:
            cursor = conn.cursor()
